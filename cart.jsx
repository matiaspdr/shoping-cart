  ...
  const restockProducts = async (url) => {
    try {
      ...
      const newItems = response.data.data.map((item) => {
        ...
        return { name, country, cost, instock: inStock };
      });

      let updatedItems = items.map((existingItem) => {
        let newItem = newItems.find((item) => item.name === existingItem.name);
        if (newItem) {
          existingItem.instock += newItem.instock;
        }
        return existingItem;
      });
